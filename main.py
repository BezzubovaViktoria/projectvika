...
class Basic(tk.Frame):
    ...
    def init_basic(self): #Добавляем значки и их функционал и заполнение интерфейса приложения 
    ...
    def search_records(self, name):
        name = "%" + name + "%"
        self.db.cursor.execute("SELECT * FROM db WHERE name LIKE ?", (name,)) #сюда передаем кортеж (name)

        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert("", "end", values=row) for row in self.db.cursor.fetchall()]


class Youths(tk.Toplevel): #Создание окна, чтобы заполнить данные в таблицу
    def __init__(self):
        super().__init__(root)
        self.init_youths()
        self.view = app
    ...
    def init_youths(self):
        ...
        label_salary = tk.Label(self, text="Зароботная плата")
...
class Update(Youths): #Создание окна для редактирования данных внесенных в таблицу пользователей  
    def __init__(self):
        super().__init__()
        self.init_edit()
        self.view = app
...
class Search(tk.Toplevel): #Создание окна для поиска сотрудников по именам и для обработки данного действия 
    def __init__(self):
        super().__init__()
        self.init_search()
        self.view = app
...
if __name__ == "__main__": #создаем приложение
    root = tk.Tk()
    db = DB()
    app = Basic(root)
    app.pack()
